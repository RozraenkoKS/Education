...
template <typename F>
void insertion_sort(F&& f, std::vector < int >& v, std::size_t l, std::size_t r) // note: o(n^2) complexity 
{
...
template <typename F>
void merge_sort(F&& f, std::vector < int >& v, std::size_t l, std::size_t m, std::size_t r)
{
...
template <typename F>
void merge_sort(F&& f, std::vector < int >& v, std::size_t l, std::size_t r) // note: o(n*log(n)) complexity (amortized)
{
...
int main()
{
	...
	auto forward_lambda = [](auto x, auto y) {
		return (x > y);
		};

	auto reverse_lambda = [](auto x, auto y) {
		return (x < y);
		};
	...
	merge_sort(forward_lambda, vector, 0, n); // good: half-open intervals preferred in c++

	for (auto element : vector) std::cout << element << ' ';
	std::cout << std::endl;

	merge_sort(reverse_lambda, vector, 0, n);
